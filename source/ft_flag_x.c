...
void	ft_flagn(unsigned int ap, t_printf *content);
void	ft_p(unsigned int ap, t_printf *content);
...
void	ft_flag_ze(t_printf *content, unsigned int ap)
{
	...
	if (content->flag_dot)
			ft_printn(content);
	ft_putnbr_base_u(ap, "0123456789abcdef", 1, content);
	content->flag_zero = 0;
}
