...
#include "../ft_printf.h"
void	ft_flagnb(char fp, unsigned int ap, t_printf *content);
void	ft_p(char fp, unsigned int ap, t_printf *content);

void	ft_printn(unsigned int ap, t_printf *content)
{
	content->cont_u = 0;
...
void	ft_flagz(char fp, t_printf *content, unsigned int ap)
{
	...
	if (content->conv[0] == '0' && content->precision <= 0)
		return ;
	if (content->flag_dot)
		ft_printn(ap, content);
	ft_putnbr_x(ap, "0123456789abcdef", fp, content);
	content->flag_zero = 0;
}
...
void	ft_flag_x(char fp, unsigned int ap, t_printf *content)
{
	...
	if (content->flag_minus)
	{
		...
		ft_flagnb(fp, ap, content);
	else if (content->flag_zero)
		ft_flagz(fp, content, ap);
