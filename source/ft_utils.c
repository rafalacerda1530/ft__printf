...
int	check_integer(char fp, int ap, t_printf *content)
{
	...
	else if (ap < 0 && fp != 'u' && fp != 'x' && fp != 'X')
	{
		...
		if (ap != INT_MIN)
			content->sub = 1;
		if (content->flag_num && content->precision == 0 && content->flag_dot
			&& content->flag_minus && content->precision == 0)
			content->width -= 2;
		else
			content->width -= 1;
	}
...
void	ft_putnbr_u(unsigned int num, char *base, char cv, t_printf *content)
{
	...
	if (cv == 'u' && content->flag_minus && content->width == 0
		&& content->flag_dot && content->u == 0)
		return ;
	else if (cv == 'u' && content->flag_minus && content->width
		&& content->flag_dot && content->u == 0)
		return ;
	if (cv == 'X')
...
void	ft_checkstring(char *ap, t_printf *content)
{
	if (!content->flag_dot || content->precision > 0)
	{
		if (content->width && content->precision > 0)
		{
			if (content->precision > content->width && !content->flag_minus)
				content->precision -= content->width;
			else if (content->precision < (int)ft_strlen((const char *)ap))
				content->width -= content->precision;
			else
